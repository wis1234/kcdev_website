...
(function() {
  ...
  if (preloader) {
    ...
    setTimeout(removePreloader, 10000);
  }
  ...
  /* Language select: update current flag icon when language changes */
  document.addEventListener('DOMContentLoaded', function() {
    function setCurrentFlag(lang) {
      var img = document.querySelector('.lang-select .current-flag') || document.querySelector('.current-flag');
      if (!img) return;
      img.src = 'assets/img/flags/' + (lang === 'en' ? 'us' : 'fr') + '.svg';
      img.alt = (lang === 'en' ? 'English' : 'Français');
      document.querySelectorAll('.lang-option').forEach(el => el.classList.toggle('active', el.dataset.langSwitch === lang));
    }
    var stored = localStorage.getItem('site-language') || 'fr';
    setCurrentFlag(stored);
